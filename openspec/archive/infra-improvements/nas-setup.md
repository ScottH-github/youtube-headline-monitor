...
### docker run 指令
```bash
/usr/local/bin/docker run --rm --stop-timeout 120 --name headline-monitor \
...
- docker run 加 `--stop-timeout 120`，確保 stop 時有足夠時間產生報告+部署
